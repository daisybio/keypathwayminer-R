...
sample_network = system.file("extdata", "sampleNetwork.sif", package="Rkpm")
# Path to colon methylation matrix
colon_methylation <- system.file(package = "Rkpm",
                                            "extdata/datasets",
...
```
You can also just provide the path to an indicator matrix file.  Assuming the matrix is provided in the right format Rkpm will automatically detect the type of input and processes it according to the execution type (local or remote):
...
sample_network = system.file("extdata", "sampleNetwork.sif", package="Rkpm")
```
For the remote execution, the user can as well select a graph_file from the web service.
```{r get_networks}
...
#### Remote
Lets take a look which URL RKPM will use for the HTPP requests.
```{r intialize}
# KeyPathWayMiner URL:
kpm_options()$url
```
...
By setting `use_range_k = TRUE` and `use_range_l = TRUE` you can provide a range for the parameters L and K.
...
**Note:** The web service does not allow individual fixed parameters to be set for each dataset at the moment. If you want to do that you will have to use local execution as explained in the next section. 
...
For the users who want to go even more into detail, we provide a properties file where the user can specify the default parameters of the local run. To access and edit the properties file use `edit(file = system.file(package = "Rkpm", "kpm.properties"))`. Keep in mind that options set with kpm_options() have higher priority than the options in the kpm.properites file.
...
**Note:** The results of the local run will be saved in the results folder on you working directory. If you to save you results in another place, change the results_folder option in the properties file.
