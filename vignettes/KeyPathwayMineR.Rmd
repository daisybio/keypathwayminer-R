...
.jaddClassPath("../inst/java/keypathwayminer-standalone-5.0.jar")
.jclassPath()
...
local_example_1 <- pathway_statistics(result = local_example_1, indicator_matrix = huntington_disease_up)

# Visualize the results with shiny
visualize_result(result = local_example_1)
```
...
visualize_result(result = local_example_2)
```
...
visualize_result(result = local_example_3)
```
When `use_range` is set to **FALSE** (by default) only `l_min` is relevant. The same applies to K (`k_min`). 
## Local
Under construction
**Important note:** The **web** service does not allow individual fixed (L case-exceptions) parameters to be set for each matrix at the moment. Robustness analysis is also not supported via Remote execution. If you want to use ranged runs and robustness analysis, please use the **Local** execution.
