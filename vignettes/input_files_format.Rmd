...
**Format:** To seperate the values **TAB seperator** is used and after every line a **NEWLINE** is included. The file is provided **without the header**. The file must be in .sif format and has the following structure:
...
**Usage: ** Once your file has the correct format provide it to Rkpm as **path** to a file.

`kpm(graph_file = PATH_TO_YOUR_NETWORK, indicator_matrices)`

## Matrix file/s
**Description:** Files containing matrices from the expression studies.
KPM can take as input **several** of these files, which can be defined either through R (Rkpm.Rmd) 
or in the DATASETS file (see below). 

**For:** `Remote` and `Local` execution.

**Format:** To seperate the values **TAB seperator** is used and after every line a **NEWLINE** is included. The file is provided **without the header**.
