...
For more information please visit the [KPM website](https://keypathwayminer.compbio.sdu.dk/keypathwayminer/) or see [Alcaraz et al. De Novo Pathway Enrichment
...
```{rnetworks_remote , echo = FALSE, cache=TRUE}
...
setwd("/nfs/home/users/michaell/KPM-R/KPM_forked/test")
# Run kpm
...
#### Local run with perturbation
In the last example, we will perform an unranged run using INES. In addition to that we will analyse the robustness of the network with the perturbation technique "nodeswap" and the following parameters:
...
local_example_3 <- kpm(graph = sample_network, indicator_matrices = huntington_disease_up)

# Visualize the results with shiny
visualize_result(local_example_3)
```
To learn more about the perturbation methods to study the robustness and validity of the extracted subnetworks please see [Alcaraz et al., F1000, 2016] (https://pubmed.ncbi.nlm.nih.gov/27540470/)
...
**Important note:** Robustness analysis is not supported via Remote execution. 
